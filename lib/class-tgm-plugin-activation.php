	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			if (TRUE !== $this->is_tgmpa_complete()) {
				...
				add_filter('update_plugin_complete_actions', array(
					$this,
					'actions'
				));
		...
		public function notices() {
			...
			if (!empty($message)) {
				...
				foreach ($message as $type => $plugin_group) {
					...
					$linked_plugins = array_map(array(
						'TGMPA_Utils',
						'wrap_in_em'
					), $linked_plugins);
...
if (!class_exists('TGMPA_List_Table')) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		 ...
		 */
		public function __construct() {
			$this->tgmpa = call_user_func(array(
				get_class($GLOBALS['tgmpa']),
				'get_instance'
			));
			...
			add_filter('tgmpa_table_data_items', array(
				$this,
				'sort_table_items'
			));
		}
		...
		protected function _gather_plugin_data() {
			...
			foreach ($plugins[$this->view_context] as $slug => $plugin) {
				...
				if (!empty($upgrade_notice)) {
					...
					add_action("tgmpa_after_plugin_row_$slug", array(
						$this,
		...
		public function process_bulk_actions() {
			...
			if ('tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action()) {
				...
				$plugins_to_install = array_map(array(
					$this->tgmpa,
					'sanitize_key'
				), $plugins_to_install);
...
if (!function_exists('tgmpa_load_bulk_installer')) {
	...
	function tgmpa_load_bulk_installer() {
		...
		$tgmpa_instance = call_user_func(array(
			get_class($GLOBALS['tgmpa']),
			'get_instance'
		));
