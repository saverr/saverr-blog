...
if (!function_exists('_mbbasetheme_posted_on')) :
	...
	function _mbbasetheme_posted_on() {
		...
		$posted_on = sprintf(_x('Posted on %s', 'post date', '_mbbasetheme'), '<a href="' . esc_url(get_permalink()) . '" rel="bookmark">' . $time_string . '</a>');

		$byline = sprintf(_x('by %s', 'post author', '_mbbasetheme'), '<span class="author vcard"><a class="url fn n" href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '">' . esc_html(get_the_author()) . '</a></span>');
